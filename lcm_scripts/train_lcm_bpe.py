...
def train_sentencepiece(
    docs: list[list[str]], out_dir: str, vocab_size: int, resume: bool = True
) -> str:
    if spm is None:
        raise RuntimeError(
    ...
    prefix_path = os.path.join(out_dir, "bhashasetu_bpe_lcm") + ".model"
    if resume and os.path.exists(prefix_path):
        # Tokenizer training is deterministic in the corpus, and the LCM
        # checkpoints were trained against *this* vocabulary — retraining it on
        # resume would silently shift every token id.
        print(f"[resume] reusing existing SentencePiece model {prefix_path}")
        return prefix_path
...
def train_epoch(
    ...
    for batch_idx, (src_ids, src_mask, tgt_ids, tgt_mask) in tqdm(
...
def main():
    ...
    sp_model = train_sentencepiece(
        train_docs, args.out_dir, args.vocab_size, resume=args.resume != "never"
    )
    ...
    # The BPE encoder is trained jointly with the LCM, so both have to be
    # checkpointed together or a resumed run would pair a half-trained encoder
    # with a fully-trained LCM.
    trainable = nn.ModuleDict({"lcm": model, "encoder": encoder})
    ckpt = TrainingCheckpointer(
        args.out_dir,
        prefix=f"lcm_bpe_fraction{args.fraction}",
        fingerprint=fingerprint,
        max_keep=args.max_checkpoints,
        save_interval_steps=args.save_interval_steps,
        save_interval_seconds=args.save_interval_seconds,
    )
    resume = ckpt.restore(ckpt.load(args.resume, map_location=device), trainable, optim)
    ...
    # work, so finished levels are replayed from the sidecar instead of rerun.
    stages = StageTracker(
