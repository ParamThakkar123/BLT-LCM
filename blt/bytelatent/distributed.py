...
# xformers may not be available on all platforms (Windows CPU builds). Do not
# import xformers.ops at module import time unguarded; we will attempt to add
# xformers-related ops to the no-recompute set below if present.
...
# Conditionally add xformers ops if they exist. On platforms without xformers
# (or when xformers C++/CUDA extensions aren't available) these attributes
# may not exist and should be skipped.
try:
    default_no_recompute_ops.add(torch.ops.xformers_flash.flash_fwd.default)
except Exception:
    # xformers flash attention not present
    pass
...
if int(os.environ.get("BLT_ALLOW_MISSING_FLEX_ATTENTION", False)) == 0:
    try:
        ...
        # missing efficient attention cutlass op; continue without it
        pass
