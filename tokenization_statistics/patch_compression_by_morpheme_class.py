...
def resolve_sentence_id(row: dict[str, Any]) -> int:
    ...
    raise ValueError("Could not find sentence index field; expected sentence_index, sentence_id, or idx")
...
def load_blt_records(blt_jsonl: str, tokenizer_path: str) -> dict[int, dict[str, Any]]:
    ...
    for row in load_jsonl(blt_jsonl):
        ...
        blt_patches = as_int(row.get("num_patches") or row.get("patches_blt"), "num_patches")
        num_bytes = as_int(row.get("num_bytes") or len(text.encode("utf-8")), "num_bytes")
        records[sentence_id] = {
...
def load_score_records(scores_jsonl: str, bpe_column: str) -> dict[int, dict[str, Any]]:
    ...
    for row in load_jsonl(scores_jsonl):
        sentence_id = resolve_sentence_id(row)
        blt_patches = as_int(row.get("patches_blt") or row.get("num_patches"), "patches_blt")
...
def load_sentence_classes(detail_jsonl: str) -> dict[int, dict[str, int]]:
    class_counts_by_sentence: dict[int, dict[str, int]] = defaultdict(lambda: defaultdict(int))
    ...
    return {sid: dict(counts) for sid, counts in class_counts_by_sentence.items()}
...
def build_analysis(
    ...
    if not matched_sentence_ids:
        raise ValueError("No overlapping sentence ids between token/patch records and morpheme detail rows")
        ...
        sentence_rows.append(
            {
                ...
                "class_word_counts": json.dumps(counts, ensure_ascii=False, sort_keys=True),
    ...
    def deltas_for(ids: list[int]) -> list[float]:
        return [float(records[sid]["blt_patches"] - records[sid]["bpe_tokens"]) for sid in ids]

    overall_ids = matched_sentence_ids
    summary["overall"] = {
        "num_sentences": len(overall_ids),
        "avg_blt_patches_per_sentence": summarize(values_for(overall_ids, "blt_patches"))["mean"],
        "avg_bpe_tokens_per_sentence": summarize(values_for(overall_ids, "bpe_tokens"))["mean"],
    ...
    for cls in CLASS_ORDER:
        ...
        class_summary = {
            ...
            "avg_blt_patches_per_sentence": summarize(values_for(ids, "blt_patches"))["mean"],
            "avg_bpe_tokens_per_sentence": summarize(values_for(ids, "bpe_tokens"))["mean"],
...
def plot_class_comparison(summary: dict[str, Any], path: str) -> None:
    ...
    blt_means = [summary["classes"][cls]["avg_blt_patches_per_sentence"] for cls in CLASS_ORDER]
    bpe_means = [summary["classes"][cls]["avg_bpe_tokens_per_sentence"] for cls in CLASS_ORDER]
    ratios = [summary["classes"][cls]["patch_to_bpe_ratio"]["mean"] for cls in CLASS_ORDER]

    x = list(range(len(CLASS_ORDER)))
    width = 0.36

    fig, ax1 = plt.subplots(figsize=(11, 5.8))
    ax1.bar([i - width / 2 for i in x], blt_means, width, label="BLT patches / sentence", color="#e74c3c")
    ax1.bar([i + width / 2 for i in x], bpe_means, width, label="BPE tokens / sentence", color="#3498db")
    ...
    for i, (blt, bpe) in enumerate(zip(blt_means, bpe_means)):
        top = max(blt, bpe)
        ax1.text(i, top + 0.5, f"{blt:.1f} / {bpe:.1f}", ha="center", va="bottom", fontsize=9)
...
def parse_args() -> argparse.Namespace:
    ...
    input_group.add_argument("--blt-jsonl", help="BLT patch JSONL with num_patches and text")
    input_group.add_argument("--scores-jsonl", help="Per-sentence scores JSONL with patches_blt and a BPE column")
    parser.add_argument("--morpheme-detail", required=True, help="fertility_by_class_detail.jsonl path")
    parser.add_argument("--bpe-tokenizer", help="tokenizer.json used to compute BPE counts for --blt-jsonl")
    parser.add_argument(
        "--bpe-column",
        default="tok_ret",
...
def main() -> None:
    ...
    json_path = os.path.join(args.output_dir, "patch_compression_by_morpheme_class.json")
    ...
    print("Patch compression ratio analysis complete")
    print(f"  Matched sentences: {summary['num_matched_sentences']}")
    print(f"  Avg BLT patches/sentence: {overall['avg_blt_patches_per_sentence']:.4f}")
    print(f"  Avg BPE tokens/sentence:  {overall['avg_bpe_tokens_per_sentence']:.4f}")
    print(f"  Mean BLT/BPE ratio:        {overall['patch_to_bpe_ratio']['mean']:.4f}")
